...
status_t FT5406_RT_Init(ft5406_rt_handle_t *handle, lpi2c_rtos_handle_t *master_rtos_handle)
{
    ...
    assert(handle);
    assert(master_rtos_handle);

    if (!handle || !master_rtos_handle)
    {
        return kStatus_InvalidArgument;
    }

    handle->rtos_handle = master_rtos_handle;
    ...
    status = LPI2C_RTOS_Transfer(master_rtos_handle, xfer);
...
status_t FT5406_RT_ReadTouchData(ft5406_rt_handle_t *handle)
{
    ...
    status = LPI2C_RTOS_Transfer(handle->rtos_handle, &handle->xfer);
